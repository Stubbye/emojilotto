...
async function generateRiddle(gameType, roundId) {
  log("Generating riddle for " + gameType + " round " + roundId);
  const prompt = gameType === "sol"
    ? `Generate a cryptic riddle for EmojiLotto lottery. Describe exactly 6 emojis from: target,rocket,gem,moon,lightning,fire,circus,butterfly,rainbow,sparkles,dice,joker,fox,dragon,flower,purple,guitar,trophy,masks,clover,crystal,star,palette,wave. Make it very hard and poetic. Return ONLY JSON: {"hardRiddle":{"text":"...","hint":"..."},"easyRiddle":{"text":"...","hint":"..."},"answer":[0,1,2,3,4,5]}`
    : `Generate a crypto memecoin themed riddle for EmojiLotto. Describe exactly 6 emojis from: target,rocket,gem,moon,lightning,fire,circus,butterfly,rainbow,sparkles,dice,joker,fox,dragon,flower,purple,guitar,trophy,masks,clover,crystal,star,palette,wave. Return ONLY JSON: {"hardRiddle":{"text":"...","hint":"..."},"easyRiddle":{"text":"...","hint":"..."},"answer":[0,1,2,3,4,5]}`;
...
async function main() {
  ...
  if (now >= state.sol.endTime) {
    ...
    log("SOL round #" + state.sol.roundId + " has " + mins + " mins left");
  }
  ...
  if (now >= state.emlo.endTime) {
    ...
    log("EMLO round #" + state.emlo.roundId + " has " + mins + " mins left");
  }
